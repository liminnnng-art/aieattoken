...
import { astDiff } from "./ast-diff/index.js";
// Initialize
...
function compileAET(code) {
    ...
    if (errors.length > 0 || !cst)
        return { go: "", error: errors.join("; ") };
    const ir = transform(cst);
    return { go: emit(ir) };
}
function runGo(goCode, tmpName) {
...
function testRoundTrip(aetCode) {
    // AET → IR (first)
    const { cst: cst1, errors: err1 } = parse(aetCode);
    if (err1.length > 0 || !cst1)
        return { pass: false, diffs: ["Parse error on first pass: " + err1.join("; ")] };
    ...
    if (err2.length > 0 || !cst2)
        return { pass: false, diffs: ["Parse error on second pass"] };
...
function generateReport(results, group) {
    const lines = [];
