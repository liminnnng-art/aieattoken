...
let aliasMap = {};
export function loadJavaAliases(path) {
    try {
...
function tokCount(node, tokenName) {
    const tokens = node.children[tokenName];
        ...
        if (Array.isArray(items)) {
            for (const item of items) {
                if (item && typeof item === "object") {
                    ...
                    if ("children" in item)
                        return getFirstTokenOffset(item);
                }
            }
        }
    }
...
// ─── Main entry point ─────────────────────────────────────────────────────────
export function transformJava(cst) {
...
function transformAtDecl(node) {
    ...
    if (rs)
        return transformRecordDecl(name, modList, typeParams, rs);
    // Interface: interfaceSuffix
    const is_ = child(node, "interfaceSuffix");
    if (is_)
        return transformInterfaceDecl(name, modList, typeParams, is_);
    // Class: classSuffix
    const cs = child(node, "classSuffix");
    if (cs)
        return transformClassDecl(name, modList, typeParams, cs);
...
// ─── Modifiers ────────────────────────────────────────────────────────────────
function transformModifiers(node) {
    const mods = [];
    // + → public
    ...
    for (let i = 0; i < absTokens.length; i++)
        mods.push("abstract");
    return mods;
}
// ─── Type params ──────────────────────────────────────────────────────────────
function transformTypeParams(node) {
    const params = [];
...
// ─── Class declaration ────────────────────────────────────────────────────────
function transformClassDecl(name, modifiers, typeParams, node) {
...
function inferClassTypeParams(body) {
...
// ─── Field declaration ────────────────────────────────────────────────────────
function transformFieldDecl(node) {
    const mods = child(node, "modifiers");
    const modList = mods ? transformModifiers(mods) : [];
...
// ─── Method declaration ───────────────────────────────────────────────────────
function transformMethodDecl(node) {
    const si = nextStmtIndex();
    const mods = child(node, "modifiers");
    const modList = mods ? transformModifiers(mods) : [];
    const name = tok(node, "Ident") || "";
    ...
    // Return type: -> Type
    const te = child(node, "typeExpr");
    const results = te ? [transformTypeExpr(te)] : [];
    // Body (optional for abstract methods)
    const blk = child(node, "block");
    const body = blk ? transformBlock(blk) : { kind: "BlockStmt", stmts: [] };
    ...
    if (modList.length > 0) {
        decl.modifiers = modList;
    }
    return decl;
}
// ─── Constructor declaration ──────────────────────────────────────────────────
function transformConstructorDecl(node) {
    const si = nextStmtIndex();
    const mods = child(node, "modifiers");
    const modList = mods ? transformModifiers(mods) : [];
    const pl = child(node, "paramList");
...
// ─── Record declaration ───────────────────────────────────────────────────────
function transformRecordDecl(name, modifiers, typeParams, node) {
    const si = nextStmtIndex();
    ...
    const interfaces = typeExprs.map(te => typeExprToString(te));
...
// ─── Enum declaration ─────────────────────────────────────────────────────────
function transformEnumDecl(node) {
    const si = nextStmtIndex();
    const mods = child(node, "modifiers");
    const modList = mods ? transformModifiers(mods) : [];
    const name = tok(node, "Ident") || "";
...
function transformEnumValues(node) {
...
function transformInterfaceDecl(name, modifiers, typeParams, node) {
    const si = nextStmtIndex();
...
function transformMethodSigList(node) {
    return children(node, "methodSig").map(ms => {
        ...
        const sig = { name, params, results };
        if (blk) {
            // Store default method body as metadata
            sig.defaultBody = transformBlock(blk);
        }
        if (mods) {
            sig.modifiers = transformModifiers(mods);
        }
        return sig;
    });
}
// ─── Top-level function or var declaration ────────────────────────────────────
function transformTopFuncOrVarDecl(node) {
    // varDeclStmt
    const vd = child(node, "varDeclStmt");
    if (vd)
        return transformVarDeclStmt(vd);
...
function transformLambdaParams(node) {
...
function typeExprToString(node) {
    // baseType child
...
function transformTypeArgEntry(node) {
...
function transformStmt(node) {
    ...
    if (c("whileStmt"))
        return transformWhileStmt(c("whileStmt"));
    if (c("tryCatchStmt"))
        return transformTryCatch(c("tryCatchStmt"));
    if (c("tryWithStmt"))
        return transformTryWith(c("tryWithStmt"));
    if (c("throwStmt"))
        return transformThrowStmt(c("throwStmt"));
    if (c("returnStmt"))
        return transformReturnStmt(c("returnStmt"));
    if (c("branchStmt"))
        return transformBranchStmt(c("branchStmt"));
    if (c("varDeclStmt"))
        return transformVarDeclStmt(c("varDeclStmt"));
    if (c("yieldStmt"))
        return transformYieldStmt(c("yieldStmt"));
    if (c("simpleStmt"))
...
function transformIfStmt(node) {
    ...
    const body = blocks[0] ? transformBlock(blocks[0]) : { kind: "BlockStmt", stmts: [] };
    let else_;
    // Recursive else-if
...
function transformForInit(node) {
    const vd = child(node, "varDeclStmt");
    if (vd)
        return transformVarDeclStmt(vd);
    const ss = child(node, "simpleStmt");
    if (ss)
        return transformSimpleStmt(ss);
    return undefined;
}
function transformForPost(node) {
    const ss = child(node, "simpleStmt");
    if (ss)
        return transformSimpleStmt(ss);
    return undefined;
...
// ─── Try-catch ────────────────────────────────────────────────────────────────
function transformTryCatch(node) {
    const si = nextStmtIndex();
    ...
    const catches = catchClauses.map(cc => {
        ...
        const catchBody = catchBlk ? transformBlock(catchBlk) : { kind: "BlockStmt", stmts: [] };
...
// ─── Try-with-resources ───────────────────────────────────────────────────────
function transformTryWith(node) {
    const si = nextStmtIndex();
    // Resources
    const resources = [];
...
// ─── Throw statement ──────────────────────────────────────────────────────────
function transformThrowStmt(node) {
    const si = nextStmtIndex();
    const expr = child(node, "expr");
    const throwExpr = expr ? transformExpr(expr) : { kind: "Ident", name: "null" };
...
function transformYieldStmt(node) {
    ...
    const expr = child(node, "expr");
    const values = expr ? [transformExpr(expr)] : [];
    const stmt = { kind: "ReturnStmt", values, stmtIndex: nextStmtIndex() };
    stmt.isYield = true;
    return stmt;
}
// ─── Branch statement ─────────────────────────────────────────────────────────
...
function transformVarDeclStmt(node) {
    const name = tok(node, "Ident") || "";
    const expr = child(node, "expr");
    const value = expr ? transformExpr(expr) : undefined;
...
function transformAssignExpr(node) {
    ...
    for (const [tokenName, opStr] of assignOps) {
        if (tok(node, tokenName)) {
            const lhs = exprs[0] ? transformExpr(exprs[0]) : { kind: "Ident", name: "_" };
            ...
            if (innerAe) {
                ...
                if (rhsNode.kind === "ExprStmt") {
                    ...
                    rhs = assignStmtToExpr(rhsNode);
                }
                else {
                    rhs = exprs[1] ? transformExpr(exprs[1]) : { kind: "Ident", name: "null" };
                }
                return { kind: "AssignStmt", lhs: [lhs], rhs: [rhs], op: opStr, stmtIndex: si };
            }
            const rhs = exprs[1] ? transformExpr(exprs[1]) : { kind: "Ident", name: "null" };
            return { kind: "AssignStmt", lhs: [lhs], rhs: [rhs], op: opStr, stmtIndex: si };
        }
    ...
    if (tok(node, "Inc")) {
        const x = exprs[0] ? transformExpr(exprs[0]) : { kind: "Ident", name: "_" };
...
function assignStmtToExpr(stmt) {
...
function transformTernaryExpr(node) {
    ...
    let result = orExpr ? transformExpr(orExpr) : { kind: "Ident", name: "_" };
    ...
    for (const op of ops) {
        if (op.type === "questionBang") {
            ...
            if (exprIdx + 1 < exprs.length) {
                ...
                result = { kind: "Java_TernaryExpr", cond: result, ifTrue, ifFalse };
                exprIdx += 2;
            }
            else {
...
function transformBinExprChain(node, childRule, opToken, opStr) {
...
function transformUnaryExpr(node) {
    ...
    if (typeExprs.length > 0 && unaryChildren.length > 0 && tok(node, "LParen")) {
        ...
        return { kind: "Java_CastExpr", type: castType, expr: inner };
    }
    // Prefix operators: +, -, !, ~
    if (unaryChildren.length > 0) {
        for (const [tokenName, opStr] of [["Plus", "+"], ["Minus", "-"], ["Bang", "!"], ["Tilde", "~"]]) {
            if (tok(node, tokenName)) {
                return { kind: "UnaryExpr", op: opStr, x: transformExpr(unaryChildren[0]) };
            }
        }
        // Pre-increment/decrement
        if (tok(node, "Inc")) {
            const inner = transformExpr(unaryChildren[0]);
            return { kind: "UnaryExpr", op: "++", x: inner };
        }
        if (tok(node, "Dec")) {
            const inner = transformExpr(unaryChildren[0]);
            return { kind: "UnaryExpr", op: "--", x: inner };
...
function transformPostfixExpr(node) {
    ...
    let result = primary ? transformExpr(primary) : { kind: "Ident", name: "_" };
    ...
    const callToExprListMap = new Map();
    {
        let ciTemp = 0;
        for (const op of ops) {
            if (op.type === "call") {
                ...
                for (const el of exprLists) {
                    const elTokens = el.children.expr || [];
        ...
        switch (op.type) {
            case "call": {
                ...
                if (result.kind === "Ident" && isUpperCase(result.name)) {
                    ...
                    if (alias && alias.isConstructor) {
                        ...
                        result = { kind: "Java_NewExpr", type: IR.simpleType(javaName), args };
                    }
                    else if (alias) {
                        ...
                        result = { kind: "Java_NewExpr", type: IR.simpleType(result.name), args };
                    }
                }
            ...
            case "index": {
                ...
                if (exprs.length > indexIdx) {
                    result = { kind: "IndexExpr", x: result, index: transformExpr(exprs[indexIdx]) };
                }
                indexIdx++;
                break;
            }
            ...
            case "methodRef": {
                ...
                result = { kind: "SelectorExpr", x: result, sel: "::" + refName };
                methodRefIdx++;
                break;
            }
            case "instanceof": {
            ...
            case "pipe": {
                ...
                if (pipeIdx < pipeOps.length) {
                    ...
                    if (javaOp === "map" || javaOp === "filter") {
                        const fn = pipeArgs[0] || { kind: "Ident", name: "_" };
                        result = {
                            kind: "PipeExpr",
                            x: result,
                            op: javaOp,
                        ...
                        const selectorExpr = { kind: "SelectorExpr", x: result, sel: javaOp };
                        result = { kind: "CallExpr", func: selectorExpr, args: pipeArgs };
                    }
                }
                pipeIdx++;
                break;
            ...
            case "postInc": {
                result = { kind: "UnaryExpr", op: "++", x: result };
                result.postfix = true;
                break;
            }
            case "postDec": {
...
function transformPrimaryExpr(node) {
    ...
    if (tok(node, "New") || child(node, "typeExpr")) {
        ...
        if (lbracks.length > 0) {
            ...
            if (sizes.length === 0) {
                sizes.push({ kind: "BasicLit", type: "INT", value: "0" });
            }
            return {
                kind: "Java_NewExpr",
        ...
        const args = el ? children(el, "expr").map(transformExpr) : [];
        return { kind: "Java_NewExpr", type: newType, args };
    }
    // 'this'
    if (tok(node, "This"))
    ...
    if (longLit)
        return { kind: "BasicLit", type: "INT", value: longLit };
    ...
    if (charLit)
        return { kind: "BasicLit", type: "CHAR", value: charLit };
...
// ─── Switch expression ───────────────────────────────────────────────────────
function transformSwitchExpr(node) {
    const exprs = children(node, "expr");
    const tag = exprs[0] ? transformExpr(exprs[0]) : { kind: "Ident", name: "_" };
    const cases = [];
...
function maybeWrapNewExpr(expr) {
    ...
    if (expr.kind === "CallExpr") {
        const call = expr;
        if (call.func.kind === "Ident" && isUpperCase(call.func.name)) {
            return {
                kind: "Java_NewExpr",
                type: IR.simpleType(call.func.name),
                args: call.args,
            };
        }
    }
...
function expandAliasToExpr(alias) {
...
function exprToJavaSource(expr) {
    switch (expr.kind) {
