 ...
 */
function collectLocalSymbols(block, symbols) {
...
function hasValueReturningSignal(n) {
    if (!n)
        return false;
    switch (n.kind) {
        case "BlockStmt": {
            ...
            for (let i = 0; i < b.stmts.length; i++) {
                ...
                if (i < b.stmts.length - 1) {
                    ...
                    if (hasValueReturningSignal(s))
                        return true;
                }
                else {
                    // Tail statement.
                    if (s.kind === "ExprStmt") {
                        if (isValueExpr(s.expr))
                            return true;
                    }
        ...
        case "IfStmt": {
            const i = n;
            if (hasValueReturningSignal(i.body))
                return true;
            if (i.else_)
                return hasValueReturningSignal(i.else_);
            return false;
        }
        case "ForStmt":
            return hasValueReturningSignal(n.body);
        case "RangeStmt":
            return hasValueReturningSignal(n.body);
        case "SwitchStmt": {
            const sw = n;
            for (const c of sw.cases) {
                ...
                if (last.kind === "ExprStmt" && isValueExpr(last.expr))
                    return true;
                if (last.kind === "ReturnStmt" && last.values.length > 0)
                    return true;
                if (hasValueReturningSignal(last))
                    return true;
            }
            return false;
        }
 ...
 */
function typeStrength(t) {
    if (!t)
        return 0;
    if (t.name === PARAM_INFER_MARKER)
        return 0;
    if (t.name === "int")
        return 1;
 ...
 */
function propagateRecursiveCallTypes(funcName, params, body) {
    if (params.length === 0 || !funcName)
        return;
    const paramNames = new Set(params.map(p => p.name));
    const calls = [];
    function collectCalls(e) {
        if (!e)
            return;
        switch (e.kind) {
            case "CallExpr": {
                const c = e;
                if (c.func.kind === "Ident" && c.func.name === funcName) {
                    calls.push(c.args);
                }
                collectCalls(c.func);
                for (const a of c.args)
 ...
 */
function convertTailExprsToReturns(block) {
    ...
    if (last.kind === "ExprStmt") {
        ...
        convertTailExprsToReturns(last);
    }
}
/**
 ...
 */
function convertInnerImplicitReturns(n) {
    if (!n)
        return;
    switch (n.kind) {
        case "IfStmt":
            convertSingleExprIfToReturn(n);
            return;
        case "SwitchStmt":
            convertSingleExprSwitchToReturns(n);
            return;
        case "ForStmt":
            for (const s of n.body.stmts)
                convertInnerImplicitReturns(s);
            return;
        case "RangeStmt":
            for (const s of n.body.stmts)
                convertInnerImplicitReturns(s);
...
function convertSingleExprIfToReturn(ifStmt) {
    ...
    for (const s of ifStmt.body.stmts) {
        if (s.kind === "IfStmt")
            convertSingleExprIfToReturn(s);
        if (s.kind === "SwitchStmt")
            convertSingleExprSwitchToReturns(s);
    }
}
/**
 ...
 */
function convertSingleExprSwitchToReturns(sw) {
    for (const c of sw.cases) {
        if (c.body.length === 1 && c.body[0].kind === "ExprStmt") {
            const expr = c.body[0].expr;
            c.body[0] = {
                kind: "ReturnStmt",
                values: [expr],
                stmtIndex: 0,
 ...
 * return statement.
 */
function convertTailExprsInIf(ifStmt) {
    convertTailExprsToReturns(ifStmt.body);
    if (ifStmt.else_) {
        if (ifStmt.else_.kind === "IfStmt") {
            convertTailExprsInIf(ifStmt.else_);
        }
        else if (ifStmt.else_.kind === "BlockStmt") {
            convertTailExprsToReturns(ifStmt.else_);
        }
    }
}
/**
 ...
 */
function convertTailExprsInSwitch(sw) {
    for (const c of sw.cases) {
        if (c.body.length === 0)
            continue;
 ...
 */
function inferReturnTypes(block, symbols) {
    const samples = [];
    ...
    switch (expr.kind) {
        ...
        case "CallExpr": {
            ...
            if (c.func.kind === "ArrayTypeExpr") {
                // []rune(x), []byte(x)
                const at = c.func;
                const eltName = at.elt.name || "byte";
                return { name: "[]" + eltName, isSlice: true, elementType: IR.simpleType(eltName) };
            }
            return undefined;
        }
        case "CompositeLit": {
            ...
            if (cl.type) {
                ...
                if (cl.type.kind === "Ident")
                    return IR.simpleType(cl.type.name);
            }
            return undefined;
        }
        case "IndexExpr": {
            ...
            if (ct) {
                ...
                if (ct.name === "string")
                    return IR.simpleType("byte");
                if (ct.isMap && ct.valueType)
                    return ct.valueType;
            }
            return undefined;
        }
        case "ParenExpr":
            return inferExprType(expr.x, symbols);
    }
    return undefined;
}
/**
 ...
 *   - otherwise → `int`
 */
function inferParamType(paramName, body, symbols) {
    let stringVotes = 0;
    let sliceVotes = 0;
    let mapVotes = 0;
    let nestedSlice = false;
    let sliceElementHint;
    function isTargetIdent(e) {
        return !!e && e.kind === "Ident" && e.name === paramName;
    }
    // Compare rune literals like 'a' or '0'.
    function isRuneLit(e) {
        return !!e && e.kind === "BasicLit" && e.type === "RUNE";
    }
    function isStringLit(e) {
        return !!e && e.kind === "BasicLit" && e.type === "STRING";
    }
    ...
    function scanIterables(n) {
        ...
        switch (n.kind) {
            ...
            case "IfStmt": {
                const i = n;
                if (i.init)
                    scanIterables(i.init);
                scanIterables(i.body);
                if (i.else_)
                    scanIterables(i.else_);
                return;
            }
            case "ForStmt": {
                const f = n;
                if (f.init)
                    scanIterables(f.init);
                if (f.post)
                    scanIterables(f.post);
    ...
    function exprDerivesFromStringParamChar(e) {
        if (e.kind === "IndexExpr" && isTargetIdent(e.x))
            return true;
        if (e.kind === "BinaryExpr") {
            const b = e;
            if (exprDerivesFromStringParamChar(b.left) || exprDerivesFromStringParamChar(b.right))
                return true;
        }
        if (e.kind === "CallExpr") {
            const c = e;
        ...
        if (e.kind === "ParenExpr")
            return exprDerivesFromStringParamChar(e.x);
        return false;
    }
    scanIterables(body);
    ...
    function walkExpr(e) {
        ...
        switch (e.kind) {
                ...
                if (c.func.kind === "SelectorExpr") {
                    ...
                    if (x.kind === "Ident" && x.name === "fmt" &&
                        ...
                        if (c.args.length > 0 && isStringLit(c.args[0])) {
                            ...
                            const verbs = [];
                            let m;
                            while ((m = verbRe.exec(fmtStr)) !== null)
                                verbs.push(m[1]);
            ...
            case "IndexExpr": {
                ...
                if (idx.x.kind === "IndexExpr" && isTargetIdent(idx.x.x)) {
                    nestedSlice = true;
                    sliceVotes += 5;
                }
                walkExpr(idx.x);
            ...
            case "BinaryExpr": {
                ...
                if (b.left.kind === "IndexExpr" && isTargetIdent(b.left.x) && isRuneLit(b.right)) {
                    stringVotes += 10;
                }
                if (b.right.kind === "IndexExpr" && isTargetIdent(b.right.x) && isRuneLit(b.left)) {
                    stringVotes += 10;
                }
                // Compared to string literal → p is string
                if (isTargetIdent(b.left) && isStringLit(b.right))
                    stringVotes += 10;
                if (isTargetIdent(b.right) && isStringLit(b.left))
                    stringVotes += 10;
                // `localVar op rune-literal` where localVar came from iterating/indexing p
                if (b.left.kind === "Ident" && runeLikeLocals.has(b.left.name) && isRuneLit(b.right)) {
                    stringVotes += 8;
                }
                if (b.right.kind === "Ident" && runeLikeLocals.has(b.right.name) && isRuneLit(b.left)) {
                    stringVotes += 8;
                }
                walkExpr(b.left);
                walkExpr(b.right);
    ...
    function walk(n) {
        ...
        switch (n.kind) {
            ...
            case "AssignStmt": {
                const a = n;
                for (const l of a.lhs) {
                    if (l.kind === "IndexExpr" && isTargetIdent(l.x)) {
                        sliceVotes += 3;
                    }
                    // p[i][j] = v → p is [][]X; rhs hints element type
                    if (l.kind === "IndexExpr" && l.x.kind === "IndexExpr" &&
                        isTargetIdent(l.x.x)) {
                        nestedSlice = true;
                        sliceVotes += 5;
                    }
    ...
    if (mapVotes > 0)
        return IR.mapType(IR.simpleType("string"), IR.simpleType("int"));
