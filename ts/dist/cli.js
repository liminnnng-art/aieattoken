...
function cmdConvert() {
    const file = args[1];
    if (!file) {
        console.error("Usage: aet convert <file.go>");
        process.exit(1);
    }
...
function cmdStats() {
    const file = args[1];
    if (!file) {
        console.error("Usage: aet stats <file.go>");
        process.exit(1);
    }
    const absPath = resolve(file);
    if (!existsSync(absPath)) {
...
function cmdCompile() {
    const file = args[1];
    if (!file) {
        console.error("Usage: aet compile <file.aet>");
        process.exit(1);
    }
...
function cmdDiff() {
    ...
    const file2 = args[2];
    if (!file1 || !file2) {
        console.error("Usage: aet diff <f1.aet> <f2.aet>");
        process.exit(1);
    }
