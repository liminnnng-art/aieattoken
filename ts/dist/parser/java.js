...
export class AETJavaParser extends CstParser {
    ...
    isAtDecl() {
        ...
        while (i < 20) {
            ...
            if (!t || t.tokenType === undefined)
                return false;
            if (tokenMatcher(t, At))
                return true;
            // Skip modifier tokens
            if (tokenMatcher(t, Plus) || tokenMatcher(t, Minus) ||
    ...
    typeParams = this.RULE("typeParams", () => {
        this.CONSUME(Lt);
    ...
    classSuffix = this.RULE("classSuffix", () => {
    ...
    // ':' = extends, '[...]' = implements
    inheritance = this.RULE("inheritance", () => {
        this.OR([
            { ALT: () => {
                    this.CONSUME(Colon);
                    this.SUBRULE(this.typeExpr);
    ...
    isMethod() {
        ...
        if (!nameToken || !tokenMatcher(nameToken, Ident))
            return false;
    ...
    constructorDecl = this.RULE("constructorDecl", () => {
        this.SUBRULE(this.modifiers);
        this.CONSUME(LParen);
    ...
    recordSuffix = this.RULE("recordSuffix", () => {
        this.CONSUME(LParen);
        this.OPTION(() => this.SUBRULE(this.paramList));
        this.CONSUME(RParen);
        // Optional implements
    ...
    interfaceSuffix = this.RULE("interfaceSuffix", () => {
        this.CONSUME(LBrack);
    ...
    interfaceBody = this.RULE("interfaceBody", () => {
        this.OR([
            // Sealed: starts with '+'
            { GATE: () => {
                    const t = this.LA(1);
    ...
    topFuncOrVarDecl = this.RULE("topFuncOrVarDecl", () => {
        this.OR([
    ...
    typeArgEntry = this.RULE("typeArgEntry", () => {
        this.OR([
            { GATE: () => {
                    const t = this.LA(1);
    ...
    isForEach() {
        ...
        while (i < 50 && depth > 0) {
            ...
            if (depth === 1 && tokenMatcher(t, Colon))
                return true;
            if (depth === 1 && tokenMatcher(t, Semi))
                return false;
            i++;
        }
        return false;
    ...
    isForEachTyped() {
        ...
        if (tokenMatcher(t2, Colon))
            return false;
        // Ident Ident => typed (Type name)
        if (tokenMatcher(t2, Ident))
            return true;
        // Ident '<' => generic type, typed
        if (tokenMatcher(t2, Lt))
            return true;
        // Ident '[' ']' => array type, typed
        if (tokenMatcher(t2, LBrack))
            return true;
        // Ident '.' => qualified type, typed
        if (tokenMatcher(t2, Dot))
            return true;
        return false;
    }
    // forInit = varDeclStmt | simpleStmt
    forInit = this.RULE("forInit", () => {
        this.OR([
            { GATE: () => {
                    const t = this.LA(1);
    ...
    tryCatchStmt = this.RULE("tryCatchStmt", () => {
        this.CONSUME(Tc);
        this.SUBRULE(this.block);
    ...
    tryWithStmt = this.RULE("tryWithStmt", () => {
        this.CONSUME(Tw);
        this.CONSUME(LParen);
    ...
    twResource = this.RULE("twResource", () => {
        this.OR([
            { GATE: () => {
                    const t = this.LA(1);
                    return t !== undefined && tokenMatcher(t, Var);
    ...
    // assignExpr = expr (assignOp assignExpr | ':=' expr | '++' | '--')?
    assignExpr = this.RULE("assignExpr", () => {
        this.SUBRULE(this.expr);
        this.OPTION(() => {
            this.OR([
                { ALT: () => {
    ...
    isTernaryQuestion() {
        const t1 = this.LA(1);
        if (!t1 || !tokenMatcher(t1, Question))
            return false;
        ...
        while (i < 60) {
            ...
            if (!t || t.tokenType === undefined)
                return false;
            if (tokenMatcher(t, LParen) || tokenMatcher(t, LBrack) || tokenMatcher(t, LBrace))
                depth++;
            if (tokenMatcher(t, RParen) || tokenMatcher(t, RBrack) || tokenMatcher(t, RBrace)) {
                depth--;
                if (depth < 0)
                    return false; // Past our scope
            }
            if (depth === 0 && tokenMatcher(t, Colon))
                return true;
    ...
    isCastExpr() {
        ...
        if (!closing || !tokenMatcher(closing, RParen))
            return false;
    ...
    isPipeOp() {
        const t1 = this.LA(1);
        if (!t1 || !tokenMatcher(t1, Pipe))
            return false;
        const t2 = this.LA(2);
        if (!t2)
            return false;
        return (tokenMatcher(t2, Mp) || tokenMatcher(t2, Flt) ||
    ...
    isBareArrayLit() {
        ...
        while (true) {
            const tb = this.LA(i);
            if (!tb || !tokenMatcher(tb, LBrack))
                break;
            const tr = this.LA(i + 1);
            if (!tr || !tokenMatcher(tr, RBrack))
                break;
            i += 2;
            hasBracket = true;
        }
        ...
        while (depth > 0 && i < 30) {
            ...
            if (tokenMatcher(t, Gt)) {
                ...
                if (depth === 0) {
                    ...
                    const after = this.LA(i + 1);
                    if (!after)
                        return true; // end of input, treat as type args
                    return (tokenMatcher(after, LParen) ||
            ...
            if (tokenMatcher(t, Semi) || tokenMatcher(t, LBrace) ||
    ...
    lambdaParams = this.RULE("lambdaParams", () => {
