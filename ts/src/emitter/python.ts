...
function emitFuncDecl(node: IR.Py_FuncDecl | IR.IRFuncDecl, level: number): string {
    ...
    let firstParamName: string | null = "self";  // null = no implicit first param
    for (const dec of decorators) {
        if (dec.expr.kind === "Ident") {
            const decName = (dec.expr as IR.IRIdent).name;
            if (decName === "classmethod") { firstParamName = "cls"; break; }
            if (decName === "staticmethod") { firstParamName = null; break; }
        }
    }
 ...
 */
function emitPyParamList(paramList: IR.Py_ParamList, isMethod: boolean, firstParamName: string | null = "self"): string {
    const parts: string[] = [];
...
function emitCompositeLit(expr: IR.IRCompositeLit): string {
    ...
    if (typeExpr.kind === "Ident") {
        const typeName = (typeExpr as IR.IRIdent).name;
        ...
        // set literal -> {...} syntax
        if (typeName === "set") {
            if (expr.elts.length === 0) return "set()";
            const elts = expr.elts.map(emitExpr).join(", ");
            return `{${elts}}`;
        }
        ...
        if (typeName === "tuple") {
            const elts = expr.elts.map(emitExpr).join(", ");
            if (expr.elts.length === 1) return `(${elts},)`;
            return `(${elts})`;
        }
...
function emitSelectorExpr(expr: IR.IRSelectorExpr): string {
    ...
    if (expr.x.kind === "Ident") {
        const modName = (expr.x as IR.IRIdent).name;
