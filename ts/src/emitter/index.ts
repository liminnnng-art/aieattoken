...
export function emit(program: IR.IRProgram): string {
  const lines: string[] = [];
...
function emitNode(node: IR.IRNode | IR.IRExprStmt, level: number): string {
  switch (node.kind) {
    ...
    default: return `${indent(level)}// unknown node: ${(node as any).kind}`;
  }
}

function emitFuncDecl(node: IR.IRFuncDecl, level: number): string {
  const parts: string[] = [indent(level), "func "];
...
function emitStructDecl(node: IR.IRStructDecl, level: number): string {
...
function emitInterfaceDecl(node: IR.IRInterfaceDecl, level: number): string {
  ...
  for (const m of node.methods) {
    ...
    let ret = "";
    if (m.results.length === 1) ret = ` ${m.results[0].name}`;
    else if (m.results.length > 1) ret = ` (${m.results.map(r => r.name).join(", ")})`;
...
function emitSwitchStmt(node: IR.IRSwitchStmt, level: number): string {
  ...
  if (node.tag) s += emitExpr(node.tag) + " ";
  s += "{\n";
...
function emitSelectStmt(node: IR.IRSelectStmt, level: number): string {
...
function emitReturnStmt(node: IR.IRReturnStmt, level: number): string {
  if (node.values.length === 0) return `${indent(level)}return`;
...
function getZeroReturnPlaceholder(): string {
...
export function emitExpr(expr: IR.IRExpr): string {
  switch (expr.kind) {
    ...
    case "FuncLit": {
      ...
      let ret = "";
      if (expr.results.length === 1) ret = ` ${expr.results[0].name}`;
      else if (expr.results.length > 1) ret = ` (${expr.results.map(r => r.name).join(", ")})`;
