...
export function astDiff(a: IR.IRProgram, b: IR.IRProgram): DiffResult {
  const diffs: string[] = [];
...
function compareDeclLists(a: IR.IRNode[], b: IR.IRNode[], path: string, diffs: string[]) {
...
function compareNodes(a: any, b: any, path: string, diffs: string[]) {
  ...
  if (typeof a !== "object") {
    ...
    if (path.endsWith(".stmtIndex")) return;
    if (a !== b) {
...
export function formatDiff(result: DiffResult): string {
  if (result.equal) return "ASTs are semantically equivalent.";
