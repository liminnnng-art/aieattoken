...
function aetjParamList(m: IR.IRFuncDecl): string {
  ...
  const entries: { name: string; typeName: string }[] = javaParams && javaParams.length > 0
 ...
 */
function tryEmbedGenericsInCtorCall(value: IR.IRExpr, lhsType: IR.IRType): string | null {
  ...
  if (value.kind === "Java_NewExpr") {
    const ne = value as IR.Java_NewExpr;
  ...
  if (value.kind === "CallExpr") {
    const call = value as IR.IRCallExpr;
    if (call.func.kind === "Ident" && (call.func as IR.IRIdent).name === lhsBase) {
...
function rhsIsMethodCall(value: IR.IRExpr): boolean {
  ...
  if (call.func.kind === "Ident") {
    ...
    if (GO_BUILTIN_CALL_NAMES.has(name)) return false;
    return true;
  }
    ...
    case "VarDecl": {
      if (node.value) {
        ...
        if (node.type && node.type.name && node.type.name.includes("<")) {
          ...
          if (pushed !== null) {
            if (_inMethodBody) return `${node.name}:=${pushed}`;
            return `var ${node.name}=${pushed}`;
          }
          if (rhsIsMethodCall(node.value)) {
            if (_inMethodBody) return `${node.name}:=${aetjExpr(node.value)}`;
