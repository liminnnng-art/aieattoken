...
function convertResultTypes(results: any): IR.IRType[] {
  ...
  return results.List.map((field: any) => convertTypeExpr(field.Type));
}
...
function convertBlockStmt(node: any): IR.IRBlockStmt {
  if (!node?.List) return { kind: "BlockStmt", stmts: [] };
...
function convertStmt(node: any): IR.IRNode | null {
  ...
  switch (node.Kind) {
    ...
    case "AssignStmt": {
      ...
      return { kind: "AssignStmt", lhs, rhs, op: node.Tok || "=", stmtIndex: 0 } as IR.IRAssignStmt;
    }
    case "ReturnStmt":
    ...
    case "IncDecStmt":
      return { kind: "IncDecStmt", x: convertExpr(node.X), op: node.Tok as "++" | "--", stmtIndex: 0 } as IR.IRIncDecStmt;
    case "SendStmt":
      return { kind: "SendStmt", chan: convertExpr(node.Chan), value: convertExpr(node.Value), stmtIndex: 0 } as IR.IRSendStmt;
    case "BranchStmt":
      return { kind: "BranchStmt", tok: (node.Tok || "break").toLowerCase(), stmtIndex: 0 } as IR.IRBranchStmt;
    case "BlockStmt":
...
function convertSwitchStmt(node: any): IR.IRSwitchStmt {
  const cases = (node.Body?.List || []).map((c: any) => ({
