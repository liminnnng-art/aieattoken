...
export class AETPythonParser extends CstParser {
  ...
  //                   | '@' 'main' '{' stmtList '}'
  private decoratedDefOrMain = this.RULE("decoratedDefOrMain", () => {
  ...
  private mainBlock = this.RULE("mainBlock", () => {
    this.CONSUME(At);
  ...
  private decoratedDef = this.RULE("decoratedDef", () => {
    this.AT_LEAST_ONE(() => {
      this.CONSUME(At);
  ...
  private decoratorExpr = this.RULE("decoratorExpr", () => {
    this.CONSUME(Ident);
    this.MANY(() => {
      this.CONSUME(Dot);
      this.CONSUME2(Ident);
  ...
  private slotsDecl = this.RULE("slotsDecl", () => {
    this.CONSUME(Slots);
    this.CONSUME(LParen);
  ...
  private asyncStmt = this.RULE("asyncStmt", () => {
    this.CONSUME(Async);
    this.OR([
      { GATE: () => this.isFuncDef(),
        ALT: () => this.SUBRULE(this.funcDef) },
  ...
  private targetList = this.RULE("targetList", () => {
  ...
  private exceptClause = this.RULE("exceptClause", () => {
    this.CONSUME(Except);
    this.OPTION(() => {
      this.SUBRULE(this.expr);
      this.OPTION2(() => {
  ...
  private withStmt = this.RULE("withStmt", () => {
    this.CONSUME(With);
  ...
  private matchStmt = this.RULE("matchStmt", () => {
    this.CONSUME(Match);
    this.SUBRULE(this.expr);
    this.CONSUME(LBrace);
  ...
  private patternAtom = this.RULE("patternAtom", () => {
    this.OR([
      // Wildcard
      { GATE: () => this.LA(1).tokenType === Underscore,
        ALT: () => this.CONSUME(Underscore) },
  ...
  private patternArgs = this.RULE("patternArgs", () => {
  ...
  private patternArg = this.RULE("patternArg", () => {
  ...
  private patternKV = this.RULE("patternKV", () => {
    this.OR([
      { GATE: () => this.LA(1).tokenType === DoubleStar,
        ALT: () => {
          this.CONSUME(DoubleStar);
  ...
  private raiseStmt = this.RULE("raiseStmt", () => {
    this.CONSUME(Raise);
    this.OPTION(() => {
      this.SUBRULE(this.expr);
      this.OPTION2(() => {
  ...
  private assertStmt = this.RULE("assertStmt", () => {
    this.CONSUME(Assert);
    this.SUBRULE(this.expr);
    this.OPTION(() => {
      this.CONSUME(Comma);
  ...
  private delStmt = this.RULE("delStmt", () => {
    this.CONSUME(Del);
    this.SUBRULE(this.expr);
    this.MANY(() => {
      this.CONSUME(Comma);
  ...
  private passStmt = this.RULE("passStmt", () => {
    this.CONSUME(Pass);
  });
  ...
  private globalStmt = this.RULE("globalStmt", () => {
    this.CONSUME(Global);
    this.SUBRULE(this.identList);
  });

  private nonlocalStmt = this.RULE("nonlocalStmt", () => {
  ...
  private assignOrExprStmt = this.RULE("assignOrExprStmt", () => {
  ...
  private augAssignOp = this.RULE("augAssignOp", () => {
    this.OR([
  ...
  private assignRHS = this.RULE("assignRHS", () => {
    this.OR([
      { GATE: () => this.LA(1).tokenType === Yield,
  ...
  private starExprList = this.RULE("starExprList", () => {
  ...
  // starExpr: '*' expr | expr
  private starExpr = this.RULE("starExpr", () => {
  ...
  private targetExprList = this.RULE("targetExprList", () => {
    this.SUBRULE(this.starExpr);
    this.MANY(() => {
      this.CONSUME(Comma);
      this.SUBRULE2(this.starExpr);
  ...
  private lambdaExpr = this.RULE("lambdaExpr", () => {
    this.CONSUME(Pipe);
  ...
  private lambdaParamList = this.RULE("lambdaParamList", () => {
  ...
  private compExpr = this.RULE("compExpr", () => {
  ...
  private orBitExpr = this.RULE("orBitExpr", () => {
    this.SUBRULE(this.xorBitExpr);
    this.MANY(() => {
      this.CONSUME(Pipe);
  ...
  private xorBitExpr = this.RULE("xorBitExpr", () => {
  ...
  private andBitExpr = this.RULE("andBitExpr", () => {
  ...
  private shiftExpr = this.RULE("shiftExpr", () => {
  ...
  private powerExpr = this.RULE("powerExpr", () => {
  ...
  private awaitExpr = this.RULE("awaitExpr", () => {
    this.OR([
      { ALT: () => {
        this.CONSUME(Await);
  ...
  private attrName = this.RULE("attrName", () => {
    this.OR([
      { ALT: () => this.CONSUME(Ident) },
  ...
  private callArgs = this.RULE("callArgs", () => {
    this.OR([
  ...
  private keywordArgName = this.RULE("keywordArgName", () => {
    this.OR([
      { ALT: () => this.CONSUME(Ident) },
      { ALT: () => this.CONSUME(Slots) },
      { ALT: () => this.CONSUME(Match) },
  ...
  private subscriptList = this.RULE("subscriptList", () => {
  ...
  private stringLit = this.RULE("stringLit", () => {
    this.OR([
  ...
  private listExpr = this.RULE("listExpr", () => {
    this.CONSUME(LBrack);
    this.OPTION(() => {
      this.SUBRULE(this.starExprList);
      // List comprehension
  ...
  private dictItems = this.RULE("dictItems", () => {
    this.SUBRULE(this.dictItem);
    this.MANY(() => {
      this.CONSUME(Comma);
  ...
  private dictItem = this.RULE("dictItem", () => {
    this.OR([
      { GATE: () => this.LA(1).tokenType === DoubleStar,
        ALT: () => {
          this.CONSUME(DoubleStar);
  ...
  private compFor = this.RULE("compFor", () => {
  ...
  private compIter = this.RULE("compIter", () => {
    this.OR([
  ...
  // compIf: 'if' orExpr
  private compIf = this.RULE("compIf", () => {
    this.CONSUME(If);
    this.SUBRULE(this.orExpr);
  });
  ...
  private yieldExpr = this.RULE("yieldExpr", () => {
    this.CONSUME(Yield);
    this.OR([
      { GATE: () => this.LA(1).tokenType === From,
        ALT: () => {
  ...
  private isMainBlock(): boolean {
    if (this.LA(1).tokenType !== At) return false;
    const t2 = this.LA(2);
    if (!t2 || t2.tokenType !== Ident) return false;
  ...
  private isLambdaExpr(): boolean {
    if (this.LA(1).tokenType !== Pipe) return false;
    ...
    while (i < 50) {
      ...
      if (t.tokenType === LBrace || t.tokenType === RBrace || t.tokenType === Semi) return false;
      if (t.tokenType === LParen || t.tokenType === LBrack) depth++;
      if (t.tokenType === RParen || t.tokenType === RBrack) depth--;
      if (depth === 0 && t.tokenType === Pipe) return true;
      i++;
    }
    return false;
  }
  ...
  private isSliceExpr(): boolean {
    let depth = 0;
    let i = 1;
    while (i < 80) {
      const t = this.LA(i);
      if (!t || t.tokenType === undefined) break;
      if (t.tokenType === LBrack || t.tokenType === LParen || t.tokenType === LBrace) depth++;
      if (t.tokenType === RBrack) {
        if (depth === 0) break;
        depth--;
      }
      if (t.tokenType === RParen || t.tokenType === RBrace) depth--;
      if (depth === 0 && t.tokenType === Colon) return true;
      i++;
    }
    return false;
  }
  ...
  private isIdentLikeToken(t: IToken): boolean {
  ...
  private isGenExprInCall(): boolean {
    let depth = 0;
    let i = 1;
    while (i < 100) {
      const t = this.LA(i);
      if (!t || t.tokenType === undefined) break;
      if (t.tokenType === LParen || t.tokenType === LBrack || t.tokenType === LBrace) depth++;
      if (t.tokenType === RParen || t.tokenType === RBrack || t.tokenType === RBrace) {
        if (depth === 0) break;
        depth--;
      }
      // 'for' at depth 0 indicates generator expression
      if (depth === 0 && t.tokenType === For) return true;
      // comma at depth 0 means it's a regular argument list
      if (depth === 0 && t.tokenType === Comma) return false;
      i++;
    }
    return false;
  }
  ...
  private isCallableType(): boolean {
...
let parser: AETPythonParser | null = null;

function getParser(): AETPythonParser {
  if (!parser) {
...
export function parsePython(code: string): { errors: string[]; cst: CstNode | null } {
