...
export class AETParser extends CstParser {
  ...
  private funcOrMethodDecl = this.RULE("funcOrMethodDecl", () => {
    this.CONSUME(Ident);
    this.OR([
      { ALT: () => {
  ...
  private hasIfInit(): boolean {
    ...
    while (i < 50) {
      const t = this.LA(i);
      if (!t) break;
      if (tokenMatcher(t, LParen) || tokenMatcher(t, LBrack)) depth++;
      if (tokenMatcher(t, RParen) || tokenMatcher(t, RBrack)) depth--;
      if (depth === 0 && tokenMatcher(t, LBrace)) break;
      if (depth === 0 && tokenMatcher(t, Semi)) return true;
      i++;
    }
    return false;
  }
  ...
  private isThreeClauseFor(): boolean {
    ...
    while (i < 50) {
      ...
      if (tokenMatcher(t, RParen) || tokenMatcher(t, RBrack)) depth--;
      if (depth === 0 && tokenMatcher(t, LBrace)) break;
      if (depth === 0 && tokenMatcher(t, Semi)) semiCount++;
      i++;
    }
    return semiCount >= 2;
  }

  private isRangeLoop(): boolean {
  ...
  private commClause = this.RULE("commClause", () => {
    this.OR([
      { ALT: () => {
        this.CONSUME(Case);
        this.SUBRULE(this.simpleStmt);
  ...
  // Defer
  private deferStmt = this.RULE("deferStmt", () => {
    this.CONSUME(Defer);
    this.OR([
  ...
  // Go statement
  private goStmt = this.RULE("goStmt", () => {
    this.CONSUME(Go);
    this.OR([
      { GATE: () => this.LA(1).tokenType === LBrace, ALT: () => {
        this.CONSUME(LBrace);
  ...
  private isSliceExpr(): boolean {
    ...
    while (depth > 0 && i < 50) {
      const t = this.LA(i);
      if (!t) break;
      if (tokenMatcher(t, LBrack)) depth++;
      if (tokenMatcher(t, RBrack)) depth--;
  ...
  private primaryExpr = this.RULE("primaryExpr", () => {
    this.OR([
      ...
      { GATE: () => {
        if (this.noCompositeLit) return false;
        const t1 = this.LA(1);
        const t2 = this.LA(2);
  ...
  private kvExpr = this.RULE("kvExpr", () => {
    this.OR([
  ...
  private litBody = this.RULE("litBody", () => {
    this.CONSUME(LBrace);
    this.OPTION2(() => {
  ...
  private compositeLit = this.RULE("compositeLit", () => {
