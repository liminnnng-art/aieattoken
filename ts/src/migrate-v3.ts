...
function isDelimiter(ch: string | undefined): boolean {
  if (ch === undefined) return true;  // start/end of segment
 ...
 */
function splitCodeAndStrings(src: string): string[] {
 ...
 */
function replaceKeywordsInCode(code: string, replacements: [string, string][]): string {
 ...
 */
function replaceVersionMarker(src: string): string {
 ...
 */
function migrateContent(src: string): [string, string[]] {
 ...
 */
function collectAetFiles(targetPath: string): string[] {
