...
function findPythonAliases(): string | undefined {
  const candidates = [
...
function findTypescriptAliases(): string | undefined {
  const candidates = [
...
function findPopularPythonAliases(): string | undefined {
  const candidates = [
...
function cmdCompile() {
  const file = args[1];
  if (!file) { console.error("Usage: aet compile <file.aet|.aetj|.aetp|.aets|.aetx> [--java] [--typed]"); process.exit(1); }
...
function cmdDiff() {
  const file1 = args[1];
  const file2 = args[2];
  if (!file1 || !file2) { console.error("Usage: aet diff <f1.aet|.aetp|.aetj> <f2>"); process.exit(1); }
...
function fileToIR(file: string, code: string): { ir?: any; error?: string } {
  const ext = extname(file).toLowerCase();
  if (ext === ".aetp" || code.startsWith("!py-v")) return compileAETPToIR(code);
  if (ext === ".aetj" || code.startsWith("!java-v")) return compileAETJToIR(code);
  if (ext === ".aets" || ext === ".aetx" || code.startsWith("!ts1") || code.startsWith("!tsx1") || code.startsWith("!ts-v") || code.startsWith("!tsx-v")) return compileAETSToIR(code);
...
function convertJavaToAETJ(javaFilePath: string, javaCode: string): string | null {
  try {
    const javaAst = parseJavaFile(javaFilePath);
    const ir = javaAstToIR(javaAst);
...
function compileAETJToIR(code: string): { ir?: ReturnType<typeof transformJava>; error?: string } {
...
function compileAETPToIR(code: string): { ir?: ReturnType<typeof transformPython>; error?: string } {
...
function convertPythonToAETP(pyFilePath: string, _pyCode: string): string | null {
  try {
...
function convertTypescriptToAET(tsFilePath: string, _tsCode: string): string | null {
  try {
...
function compileAETSToIR(code: string): { ir?: any; error?: string } {
  const parseResult = parseTypescriptAET(code);
  if (parseResult.errors.length > 0) {
    return { error: `AET-TS parse error: ${parseResult.errors.join("; ")}` };
  }
  if (!parseResult.ir) return { error: "No IR produced" };
